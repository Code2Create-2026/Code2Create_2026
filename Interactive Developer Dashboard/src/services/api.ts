...
export const analyzeProject = async (file?: File, samplePath?: string): Promise<AnalysisResponse> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      if (samplePath === 'sample_project_java') resolve(mockSampleResponseJava);
      else if (samplePath === 'sample_project_cpp') resolve(mockSampleResponseCpp);
      else resolve(mockSampleResponsePython);
    }, 2500);
  });
};